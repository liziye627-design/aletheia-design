...
def get_search_id() -> str:
    """Generate search ID for XiaoHongShu search API."""
...
def get_img_url_by_trace_id(trace_id: str, format_type: str = "png") -> str:
    """Get image URL by trace ID."""
    return f"{random.choice(IMG_CDNS)}/{trace_id}?imageView2/format/{format_type}"


def get_img_urls_by_trace_id(trace_id: str, format_type: str = "png") -> List[str]:
    """Get all image URLs by trace ID (all CDNs)."""
    return [f"{cdn}/{trace_id}?imageView2/format/{format_type}" for cdn in IMG_CDNS]


def get_trace_id_from_url(img_url: str) -> str:
...
def parse_creator_info_from_url(url: str) -> CreatorUrlInfo:
    ...
    Supports:
        1. Full URL: "https://www.xiaohongshu.com/user/profile/5eb8e1d400000000010075ae?xsec_token=..."
    ...
    """
    # If it's a pure ID format (24 hexadecimal characters)
...
def sign(a1: str = "", b1: str = "", x_s: str = "", x_t: str = "") -> Dict[str, str]:
    ...
    encode_str = encode_utf8(json.dumps(common, separators=(',', ':')))
    x_s_common = b64_encode(encode_str)
