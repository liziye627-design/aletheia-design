...
class WeiboLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if self._config.LOGIN_TYPE == "qrcode":
            ...
            raise ValueError("[WeiboLogin.begin] Invalid Login Type. Supported: qrcode, phone, cookie")
    ...
    async def check_login_state(self) -> bool:
        ...
        return bool(cookie_dict.get("SUB"))

    async def login_by_qrcode(self):
        """Login by QR code scanning."""
        logger.info("[WeiboLogin.login_by_qrcode] Begin QR code login...")
        qrcode_img_selector = "xpath=//img[@class='cover'"
        ...
        try:
            ...
            logger.info("[WeiboLogin.login_by_qrcode] Login failed")
            sys.exit()

        logger.info("[WeiboLogin.login_by_qrcode] Login successful")
        await asyncio.sleep(5)

    async def login_by_mobile(self):
        """Login by mobile phone number."""
        logger.info("[WeiboLogin.login_by_mobile] Mobile login not fully implemented")

    async def login_by_cookies(self):
        """Login using saved cookies."""
        logger.info("[WeiboLogin.login_by_cookies] Begin cookie login...")
