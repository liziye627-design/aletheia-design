...
面向 AI 时代舆情与事实核验场景的一体化调查系统（Backend + Frontend + 多源采集）。
...
- `archive/`：历史与归档内容（当前仅保留 `.kiro` 规格历史）。

## 仓库状态（已去重）

- 已移除重复上传的 `archive/core-components` 镜像目录。
- 当前主干以 `frontend + aletheia-backend + docs + scripts` 为核心交付。
